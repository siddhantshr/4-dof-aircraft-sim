...
class Aircraft:
    ...
    Args:
        ...
        initial_state (list): A list containing the initial body x-axis velocity
            `u`, body z-axis velocity `w`, pitch rate `q`, pitch angle `theta`, height `height`, and x-position `x`.
    """
    def __init__(self, initial_state) -> None:
        ...
        self.radius_of_gyration_yy = None # Estimated radius of gyration in the y-axis as a fraction of the aircraft length
    ...
    # """Prints a warning message in red color to indicate potential issues or important information."""
    #     print(f"\033[91m{message}\033[0m")

    # def check_low_speed_aerodynamics(self):
    # """Checks if the true airspeed of the aircraft is within the low-speed aerodynamics regime (M < 0.3) and prints a warning if it exceeds this limit."""
    ...
    def initialize_config(self, PARENT_PATH, airfoil_file, config_file) -> None:
        ...
        Args:
            ...
            geometry_file (str): The filename of the geometry data (JSON).
            drag_polar_file (str): The filename of the drag polar data (CSV).
        ...
        with open(PARENT_PATH / config_file, 'r') as file:
            ...
            self.radius_of_gyration_yy = float(config["B737"]["geometry"]["radius_of_gyration_yy"])
            self.L = float(config["B737"]["geometry"]["length"])
            self.drag_severity_multiplier = float(config["B737"]["wave_drag"]["severity_multiplier"])
    ...
    def print_state(self) -> None:
        ...
        mach = v / np.sqrt(1.4 * self.atmosphere.R * (self.atmosphere.T0 + self.atmosphere.L * self.height))
    ...
    def derivatives(self, t, state) -> list:
        ...
        if np.degrees(alpha) > self.airfoil.stall_angle or np.degrees(alpha) < self.airfoil.negative_stall_angle:
            raise StallError(
                np.degrees(alpha), self.airfoil.stall_angle
            )
        v = np.sqrt(u**2 + w**2)  # True airspeed
        mach = v / np.sqrt(1.4 * self.atmosphere.R * (self.atmosphere.T0 + self.atmosphere.L * height))
        if mach >= 1.0:
            raise SupersonicFlowError(mach)
        beta = np.sqrt(1 - mach**2)
    
        cL = self.airfoil.cl_slope * (np.degrees(alpha) - self.airfoil.alpha0) # cl_slope is in per degree
        if mach > 0.3:
            cL /= beta
        cD = (
        ...
        if mach > 0.7:
            ...
            cD += self.drag_severity_multiplier * (mach - self.mcrit)**self.curve_exponent
        ...
        u_dot = (T-D*np.cos(alpha) + L*np.sin(alpha)) / self.m - self.atmosphere.g * np.sin(theta) - q*w
        w_dot = (0-D*np.sin(alpha) - L*np.cos(alpha)) / self.m + self.atmosphere.g * np.cos(theta) + q*u
        q_dot = M / self.iyy
        theta_dot = q
