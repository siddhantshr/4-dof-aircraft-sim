...
class CriticalMachWarning(Warning):
    """Warning raised when the Mach number approaches the critical Mach number (M > 0.7)."""
...
class StallError(Exception):
    ...
    def __init__(self, alpha: float, stall_angle: float) -> None:
        ...
        super().__init__(
            f"Stall condition detected: Angle of attack {alpha:.2f}° exceeds stall angle {stall_angle:.2f}°"
        )
...
class GroundContactError(Exception):
    ...
    def __init__(self, height: float) -> None:
        self.height = height
        super().__init__(
            f"Ground contact detected: Aircraft altitude {height:.2f} m is at or below ground level."
        )